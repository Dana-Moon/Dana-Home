...
let banner = {
    ...
    rollInit: function (newinterval) {
        ...
        //현재 배너
        let firstitem = document.querySelector('.rollimgs div');
        if(firstitem) {
            firstitem.classList.add('currentroll');
        }
        //다음 배너
        let seconditem = document.querySelectorAll('.rollimgs div')[1];
        if(seconditem) {
            seconditem.classList.add('nextroll');
        }
        //이전 배너
        document.querySelector('.rollimgs div:last-child').classList.add('prevroll');
    ...
    rollNext: function () {
        ...
        if(document.querySelector('.currentroll').nextElementSibling) {
            ...
            document.querySelector('.rollimgs div').classList.add('nextroll');
        }
    }
}
